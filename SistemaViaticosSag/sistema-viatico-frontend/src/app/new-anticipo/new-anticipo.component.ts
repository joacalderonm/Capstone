...
import { AnticipoCreate } from '../interfaces/plantilla';

@Component({
  selector: 'app-new-anticipo',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './new-anticipo.component.html',
  styleUrls: ['./new-anticipo.component.css']
})
export class NewAnticipoComponent implements OnInit {
  @Input() idPlantilla!: number;
  @Output() anticipoCreated = new EventEmitter<void>();
  ...
  private initForm(): void {
    ...
    this.anticipoForm.get('dias_100')?.valueChanges.subscribe(value => {
      if (value === null || value === undefined || value === '') {
        this.anticipoForm.patchValue({ dias_100: 0 });
      }
    });

    this.anticipoForm.get('dias_40')?.valueChanges.subscribe(value => {
      if (value === null || value === undefined || value === '') {
        this.anticipoForm.patchValue({ dias_40: 0 });
      }
    });
  }
  ...
  crearAnticipo(): void {
    if (this.anticipoForm.valid && this.idPlantilla) {
      ...
      const anticipoData: AnticipoCreate = {
      ...
      this.apiService.createAnticipo(anticipoData).subscribe({
        next: (response) => {
          console.log('Anticipo creado exitosamente:', response);
          this.resetForm();
          // Emitir evento para actualizar la lista de anticipos
          this.anticipoCreated.emit();
        },
        error: (error) => {
          console.error('Error al crear anticipo:', error);
          const errorMessage = error.error?.detail || 'Error al crear el anticipo. Por favor, inténtelo nuevamente.';
          alert(errorMessage);
        }
      });
    } else {
  ...
  private markFormGroupTouched(): void {
    Object.values(this.anticipoForm.controls).forEach(control => {
      control.markAsTouched();
    });
  }

  getFieldError(fieldName: string): string {
    const field = this.anticipoForm.get(fieldName);
